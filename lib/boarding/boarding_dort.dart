...
class _StateBoardinDort extends State<BoardinDort> {
  ...
  String _selectedIlId = ""; // Seçili il'in ID'si
  String _selectedIl = "İzmir"; // İlk seçili il adı
...
Future<void> _fetchIl() async {
    try {
      ...
      if (result.success && result.result != null) {
        ...
        setState(() {
          ilList = responseData.map<String>((item) => item['il_adi'] as String).toList();
          ilIdList = responseData.map<String>((item) => item['il_id'] as String).toList();
  ...
  Future<void> _fetchIlce() async {
    try {
      ...
      final Map<String, dynamic> params = <String, dynamic>{'ilId': _selectedIlId}; // ID kullan
      final ParseResponse result = await function.execute(parameters: params);

      if (result.success && result.result != null) {
        final responseData = result.result as List<dynamic>;
        setState(() {
          ilceList = responseData.map<String>((item) => item['ilce_adi'] as String).toList();
        });
        print(ilceList);
      } else {
        throw Exception('Sunucu tarafında bir hata oluştu.');
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (String? newValue) {
                              setState(() {
                                ...
                                _selectedIlId = ilIdList[ilList.indexOf(_selectedIl)]; // ID'yi güncelle
                            ...
                            items: ilList.isEmpty ? [] : ilList
                              ...
                              return DropdownMenuItem<String>(
                                ...
                                child: Text(
                                  value,
                                  style: TextStyle(
                                    color: value == 'bos' ? Colors.grey : Colors.black, // Placeholder ve diğer seçeneklerin renkleri
                                  ),
                                ),
                              );
                            }).toList(),
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            items: ilceList.isEmpty ? [] : ilceList
                                .map<DropdownMenuItem<String>>((String value) {
                              return DropdownMenuItem<String>(
                                value: value,
                                child: Text(
                                  value,
                                  style: TextStyle(
                                    color: Colors.black, // İlçelerin renkleri
                                  ),
                                ),
                              );
                            }).toList(),
